...
export async function sha256(value: string): Promise<string> {
  ...
  return pureSha256(value);
}

// Small dependency-free fallback for runtimes without Web Crypto. It is only used
// for the local request fingerprint; no prompt content is ever emitted by the core.
function pureSha256(value: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + s1 + ch + (constants[i] ?? 0) + (words[i] ?? 0)) >>> 0;
